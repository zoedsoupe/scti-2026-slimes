"""Testes da política de retry da infraestrutura sobre o mapa de pendentes
(at-least-once + idempotência). ack remove; timeout dentro do orçamento
reenvia; no limite do orçamento descarta.
...
class TestPending(unittest.TestCase):
    def test_ack_remove_o_ref_dos_pendentes(self):
        pending = add_pending({}, "r-1", "ACT r-1 pass", 10)
        pending = on_ack(pending, "r-1")
        self.assertNotIn("r-1", pending)

    def test_timeout_dentro_do_orcamento_gera_retry(self):
    ...
    def test_timeout_no_limite_do_orcamento_descarta(self):
    ...
    def test_ref_recente_nao_vence(self):
        pending = add_pending({}, "r-1", "ACT r-1 pass", 10)
