 ...
 * A ação de domínio é {"expand" | "attack" | "fortify", x, y} ou {"pass"}.
 * Compare action->kind com strcmp; escreva com snprintf(out, cap, ...).
 ...
 * (parse_cell e parse_cells acima já existem, use-as; split e to_int
 * também). Devolva 0 com `out` preenchida (type = MSG_OBS, ref, tick,
 * status, scores_tick, cells, n_cells) ou -1 com o motivo em out->error
 * (o helper err acima faz isso). Linha malformada vira erro, nunca crash.
 * Tokens extras no final são ignorados (é o que salva o seu cliente no
 * drill da v2). Atenção: tokenizar destrói a string, então copie `line`
 * para um buffer próprio antes de chamar split (veja como parse_line faz).
 * Enquanto o stub estiver aqui toda OBS chega como malformada. */
int parse_observation(const char *line, Msg *out) {
  (void)line;
  (void)parse_cells; /* silencia o warning até você usar no E1 */
  return err(out, "TODO E1: implemente parse_observation");
}
