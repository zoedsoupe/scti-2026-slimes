defmodule SlimesClient.Protocol do
  ...
  def encode_action(%{kind: "pass"}, ref), do: "ACT #{ref} pass"

  def encode_action(%{kind: kind, x: x, y: y}, ref),
    do: "ACT #{ref} #{kind} #{x} #{y}"
  ...
  defp parse_observation(t) do
    with {:ok, tick} <- int(tok(t, 2)),
      ...
      :error -> {:error, "observacao malformada"}
      status when is_binary(status) -> {:error, "status invalido #{status}"}
    end
  end
