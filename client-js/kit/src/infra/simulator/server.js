...
import { parseLine, parseCell } from "../../protocol/parse.js";
import { encodeHello, createRefs } from "../../protocol/encode.js";
...
export function createSimServer(opts = {}) {
  ...
  // codec próprio dos bots: parseObservation (E1) e encodeAction (E2) são
  // exercícios do estudante neste kit, então os bots não podem usá-los
  function botParseObs(line) {
    const t = line.split(" ");
    const cells = t[5] ? t[5].split(";").map(parseCell) : [];
    return { tick: Number(t[2]), status: t[3], cells };
  }
  const fmtAct = (action, ref) =>
    action.kind === "pass"
      ? `ACT ${ref} pass`
      : `ACT ${ref} ${action.kind} ${action.x} ${action.y}`;
  ...
  function bootBot(botName) {
    ...
    endpoint.onLine((line) => {
      if (line.startsWith("OBS ")) {
        if (myId === null) return;
        const obs = botParseObs(line);
        if (obs.status !== "alive") return;
        const [action, nextRng] = strategy(obs, myId, rng);
        rng = nextRng;
        endpoint.sendLine(fmtAct(action, refs.next()));
        return;
      }
      const parsed = parseLine(line);
      if (parsed.tag !== "ok") return;
      const msg = parsed.value;
      if (msg.type === "welcome" && msg.role === "colony") myId = msg.id;
    });
